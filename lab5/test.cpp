...
bool normalDir(GLfloat lightPosition, nx, ny, nz){
	return lightPosition[0] * nx + lightPosition[1] * ny + lightPosition[2] * nz > 0;
}

// Функция для вычислениZZя нормали на основе трёх точек
...
void drawNormalDirection(float x, float y, float z, float nx, float ny, float nz){
	glDisable(GL_LIGHTING);
	glColor3f(1.0f,1.0f,0.0f);
	glBegin(GL_LINES);
	glVertex3f(n,y,z);
	glVertex3f(x+nx*0.5f, y+ny*0.5f, y+ny*0.5f);
	glEnd();
...
void drawCircleTriangles(float radius, int segments, float height, bool upward) {
    ...
    for (int i = 0; i < segments; ++i) {
		...
		drawNormalDirection(x1,height,z1,nx,ny,nz);
		drawNormalDirection(x2,height,z2,nx,ny,nz);
    }
    glEnd();
}
...
void drawCylinder(float radius, float height, int segments) {
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        drawNormalDirection(x1, 0.0f, z1, nx, ny, nz); // Отрисовка направления нормали
        drawNormalDirection(x1, height, z1, nx, ny, nz);
        drawNormalDirection(x2, height, z2, nx, ny, nz);
        // Треугольник 2
        glVertex3f(x1, 0.0f, z1);   // Низ первой точки
        ...
        drawNormalDirection(x2, 0.0f, z2, nx, ny, nz); // Отрисовка направления нормали
	}
    glEnd();
